...
// int pushwpl(int* brd, long* bits, int** brdcpy, int** nalscpy, long** bitscpy) {
...
int** solveSudoku(int** origmat) {
	...
	for(idx=0; idx<BOARD_SIZE; ++idx) {
		...
		x = bit2nal(bits[idx]);
		if (wpl_i_sz[x]==0) {
			wplhelpers[x] = malloc(BOARD_SIZE*sizeof(int));
		}
		...
		for(jdx=0; (jdx<wpl_i_sz[idx]) && (fl_brk==0); ++jdx) {
			for(vit=1; vit<=SIZE; ++vit) {
				...
				if( bits[tochng] & (1<<vit)) {
					...
					set_board_value(newbrd, newbits, NULL, tochng, vit);
...
int hdfs(int* brd,long* bits, int* base){
	...
	for(val_iter = 1; val_iter <= SIZE; ++val_iter){
		if(oldbits & (1<<val_iter)){
			...
			nalters = set_board_value(brd,bits,alters,min_idx,val_iter);
			mysolved = hdfs(brd,bits, base);
			if(mysolved == SOLVED){
				free(lbrd);
				// free(lnals);
				free(lbits);
			...
			undo_alterations(brd,bits,alters,nalters,val_iter);
		}
	}
...
int set_board_value(int* brd, long* bits, int* alters,int idx,int val ){
	int nalters = -1;
	int r,c,i,rp,cp;
	r = idx / SIZE;
	c = idx % SIZE;
...
void undo_alterations(int* brd, long* bits, int* alters, int nalters,int val){
	int bitval = 1<<val;
	while(nalters > -1){
...
int hs(int* brd,long* bits, int* base){
...
eliminate:
		...
		for(idx = 0; idx < BOARD_SIZE;++idx){
			...
			if(bits[idx] == 0){
				...
				set_board_value(brd,bits,NULL,idx,ctr); 
				fl_changed = 1; 
		...
		for(idx=0; idx<(3*BOARD_SIZE); ++idx) {
			...
			if ( b>0 ) {
				...
				if(brd[b]==0 && (bits[b] & (1<<val))) {
					fl_changed = 1;
					set_board_value(brd, bits, NULL, b,val);
				}
			}
		}
		if(fl_changed) {
