		...
		for(hma=0; hma<=SIZE; ++hma) {
			...
			for( idx_hma=0; idx_hma < initialization_helper_sizes[hma]; ++idx_hma) {
				...
				for(idx_cell_list=1; idx_cell_list<=board[branch_on].n_allowed; ++idx_cell_list) {
					...
					mypushes++;
					int val = newboard[branch_on].list[idx_cell_list];
					set_value(&((newboard)[branch_on]), val);
					int r = (branch_on - 1) / SIZE;
					int c = (branch_on - 1) % SIZE;
					int r_prime = r - (r % MINIGRIDSIZE);
				  	...
				  	for(jdx = 0;jdx<SIZE;++jdx){
					  	...
					  	remove_allowed(&newboard[jdx_r],val);
					  	remove_allowed(&newboard[jdx_c],val); 
					  	remove_allowed(&newboard[jdx_b],val);
				  	}
